...
class BubbleSortTest(custom_test_cases.SortingTestCase):
  ...
  def test_reversedList(self):
    ...
    self.assertEqual(bubble.bubble(reversed_list), sorted(reversed_list))

  def test_sortedList(self):
    sorted_list = self.create_sorted_list(100)
    self.assertEqual(bubble.bubble(sorted_list), sorted(sorted_list))
  ...
  def test_shuffledList(self):
    ...
    self.assertEqual(bubble.bubble(shuffled_list), sorted(shuffled_list))
